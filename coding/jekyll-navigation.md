...
### Параметр ```PINNED```
Указывает нужно ли перед основным списков вывести список закрепленных в отдельным блоке ```<ul>```. Что бы это работало, параметр должен быть не пустой. Но если в нем передать строку "only", то выведется только закреп, а основной список будет скрыт. 

Чтобы добавить страницу в закрепленные - в ней нужно прописать метку ```pin: 1```. Закрепы выводятся в основном списке со скрепкой впереди. Если у страницы задана метка```pin: 0```, то она вообще игнорируется.

### Параметр ```DATE```
По умолчанию рядом с каждой ссылкой выводится ее дата публикации. Чтобы ее скрыть, в параметре нужно передать строку "no"
