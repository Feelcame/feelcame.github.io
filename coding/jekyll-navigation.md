...
В любом месте страницы можно написать одну строчку и она выведет список всех страниц в папке. Поддерживается кастомизация поведения через параметры (о них ниже). 

**Использование:**
Простой пример: {% raw %}
```{% include ls.md %}```{% endraw %} 
...
Данный скрипт был написан до того, как я обнаружил туториал от Jekyll: [как сделать навигацию](https://jekyllrb.com/tutorials/navigation/). Но это и хорошо - у меня получилось лучше 👍. Но все-равно зайди почитай


**Сортировка** страниц по дате (от нового к старому). Страницы без даты выводятся в самом низу отдельно и сортируются по алфавиту. По-сути это два списка

**Группировка** по пять ссылок, начиная от самой старой записи.
...
- ```hide_for_all``` - не добавлять метки времени, даже если они прописаны в заголовке страницы (не реализовано)
