...
Можно добавлять подсветку кода. Для этого после трех кавычек нужно написать название языка {% raw %} ``` html{% endraw %} 
...
И еще один вариант - однострочный комментарий. Представляет из себя объявнение безымянной переменной, которая нигде не используется и не выводится.
```
[]: This is comment
```
