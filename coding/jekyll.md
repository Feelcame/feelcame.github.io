...
## Что оно делает
**Jekyll** преобразовывает человекочитаемые markdown-тексты в обычный HTML. У него есть [аналоги](./hosting-cms.md#cms-static), в том числе более удобные.  
...
На [GitHub Pages](https://pages.github.com/), который предоставляет энтузиастам **бесплатный хостинг для документации**, используется именно Jekyll. Грех этим не пользоваться! Этот сайт тоже хостится на гитхабе. На github используется парсер md-файлов [kramdown](https://devhints.io/kramdown)
...
К сожалению, официальная документация очень куцо описывает функционал кастомизации. И дается очень мало примеров, как надо делать. Информацию буквально приходится выкусывать кусочками из разных источников по кусочкам. Но я смог собрать список полезных ресурсов для изучения большинства фич данной CMS, и ссылки на инфу добавил в самом низу страницы в разделе #полезные-ссылки [тык](#полезные-ссылки) <#полезные-ссылки>
