...
Есть несколько вариантов как дклать вставки кода:
- Тройной символ обратного ударения ( **\`\`\`** )
- четыре поробела перед каждой строчкой кода
- Блок highlight. {% raw %}{% highlight ruby linenos %} code {% endhighlight %}{% endraw %}

**Добавление трех символов обратного ударения ( \`\`\` )**  
Добавлять нужно в начале и в конце блока кода. Самый простой вариант.

Что бы подсветить код, после трех кавычек нужно написать название языка ( **\`\`\`** html ).  
...
### Экраниерование
1. Обратный слеш для экранирования одного символа ```\\```. Распространяется на большинство служебных символов: \* \- \`
2. Блок ```raw``` для экранирования блоков текста.

Если нужно экранировать теск или код, который содержит двойные фигурные скобки или Liquid скрипт, нужно его обернуть в блок raw.  
