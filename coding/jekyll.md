...
Если нужно добавить код который содержит двойные фигурные скобки или Liquid скрипт, то это нужно сделать в блоке raw:  
```{ % raw % } ... { % endraw % }```.  
...
```
{% endraw %}
Пример ткого комментария внутри скобок: 
({% comment %} Этот текст никогда не попадет на страницу {% endcomment %}).  
